...
var Obj3D			= function()
{
	...
	{
		var percent = main.numLoaded / (main.numTotal - 1) * 100;

		$("#s_load").html(main.numLoaded);
		$("#s_total").html(main.numTotal - 1);
		...
		if(main.numTotal - 1 >= main.numLoaded)
		{
			...
			var mtl 	= main.objArr[main.numLoaded].mtl;
			...
			var obj_dir	= main.objArr[main.numLoaded].obj_dir;

			main.numLoaded ++;
			main.placeOBJ(obj,mtl,type,x,y,width,height,depth,color,angle,pArr,obj_dir)
		}
		else
		{
			$("#overlay").fadeOut();
	...
	main.placeOBJ		= function(obj,mtl,type,x,y,width,height,depth,obj_color,angle,pointArr,obj_dir)
	{
		var mtl_color = obj_color;

		if(obj) obj = "objs/" + obj_dir + "/" + obj;
		if(mtl) mtl = "objs/" + obj_dir + "/" + mtl;

		if(obj_color == "none") obj_color = 0xFFFFFF;
		...
		if(!obj)
		{
			...
			switch(type)
			{
				...
				case "circle" :
					...
					geometry = new THREE.CylinderGeometry( ratio / 2, ratio / 2, depth, 30,4 );
					object   = new THREE.Mesh( geometry, material );

					object.scale.x = width / ratio;
					object.scale.y = height  /ratio;
				break;
		...
		else
		{
			...
			{
				main.loader.load( obj, mtl, function ( object )
				{
					object.traverse (function (child)
					{
					    var material = new THREE.MeshPhongMaterial(
